...
public class FragmentFile extends FragmentBase implements OnItemClickListener,
	...
	public void update(int flag, POMedia media) {
		...
		case MediaScannerService.SCAN_STATUS_START:
			...
			// FIXME 这个分支在首次扫描、MainActivity启动
			// service时不会被调用，因为Fragment的对象是在bindService
		...
		case MediaScannerService.SCAN_STATUS_END:// 扫描完成
			...
			if (getActivity() == null)
				break;
			Toast.makeText((MainActivity) getActivity(), "SD卡扫描完毕",
	...
	// mDownloadAdapter = new FileAdapter(getActivity(), new
	// ArrayList<POMedia>());
	...
	private class FileAdapter extends ArrayAdapter<POMedia> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			// 显示进度表
			// final ImageView status = (ImageView)
