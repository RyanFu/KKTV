...
/* --------------------------------------------------- */
/* add by juguofeng	2012-12-25
  * for YUV (format: YUV420P) picture save
  */
//#define ENABLE_YUV420P_PIC_SAVE

#ifdef ENABLE_YUV420P_PIC_SAVE
static char yuv_name[50] = {0};
static int yuv_num = 0;
static FILE *pout = NULL;

void save_yuv420p_pic(picture_t *decoded)
{
	int i;
	int height = decoded->format.i_height;
	int width = decoded->format.i_width;
	
	plane_t y_plane = decoded->p[0];
	plane_t u_plane = decoded->p[1];
	plane_t v_plane = decoded->p[2];

	sprintf(yuv_name, "/mnt/sdcard/Movies/%d.yuv", yuv_num++);
	pout = fopen(yuv_name, "wb");
	if (pout == NULL)
		__android_log_print(ANDROID_LOG_ERROR, "hoperun","file:%s--func:%s--line:%d --->>>  file open failed", __FILE__, __func__, __LINE__);
	
	if (pout) {
		/* save Y plannar */
		for (i = 0; i < height; i++)
			fwrite(y_plane.p_pixels + i * y_plane.i_pitch, 1, width, pout);
		/* save U plannar */
		for (i = 0; i < height / 2; i++)
			fwrite(u_plane.p_pixels + i * u_plane.i_pitch, 1, width / 2, pout);
		/* save V plannar */
		for (i = 0; i < height / 2; i++)
			fwrite(v_plane.p_pixels + i * v_plane.i_pitch, 1, width / 2, pout);
		/* flush to sdcard file */
		fflush(pout);
		fclose(pout);
		pout = NULL;
	}
...
/* --------------------------------------------------- */
/* add by juguofeng	2012-12-25
  * for YUV (format: ) picture save
  */
  __android_log_print(ANDROID_LOG_DEBUG, "hoperun","pic");
#ifdef ENABLE_YUV420P_PIC_SAVE
 	 save_yuv420p_pic(decoded);
#endif
/* --------------------------------------------------- */
