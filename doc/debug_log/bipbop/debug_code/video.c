...
__android_log_print(ANDROID_LOG_DEBUG, "hoperun","--1--p_sys->i_pts  = %lld", p_sys->i_pts );
#ifndef ANDROID
    ...
    while( p_block->i_buffer > 0 || p_sys->b_flush )
    {
        ...
        picture_t *p_pic;
        AVPacket pkt;
__android_log_print(ANDROID_LOG_DEBUG, "hoperun","--1-- p_block->i_pts  = %lld",  p_block->i_pts );
...
__android_log_print(ANDROID_LOG_DEBUG, "hoperun","--2-- p_block->i_pts  = %lld",  p_block->i_pts );
        post_mt( p_sys );
        ...
        if( !b_gotpicture )
        {
          __android_log_print(ANDROID_LOG_DEBUG, "hoperun","Nothing to display");
            if( i_used == 0 ) break;
            continue;
        }
        ...
        if( p_sys->p_ff_pic->reordered_opaque != INT64_MIN )
        {
            ...
            if( b_dts )
            {
                if( !p_context->has_b_frames ||
                    ...
                    p_sys->i_pts <= VLC_TS_INVALID )
                    i_pts = i_ts;
__android_log_print(ANDROID_LOG_DEBUG, "hoperun","--1-- i_pts  = %lld",  i_pts );
        ...
        if( i_pts <= VLC_TS_INVALID ) {
            i_pts = p_sys->i_pts;
__android_log_print(ANDROID_LOG_DEBUG, "hoperun","--1_0-- i_pts  = %lld",  i_pts );
        	}

__android_log_print(ANDROID_LOG_DEBUG, "hoperun","--2-- i_pts  = %lld",  i_pts );
...
__android_log_print(ANDROID_LOG_DEBUG, "hoperun","--3-- i_pts  = %lld",  i_pts );
        ...
        {
        __android_log_print(ANDROID_LOG_DEBUG, "hoperun","ready display");
...
__android_log_print(ANDROID_LOG_DEBUG, "hoperun","--4-- i_pts  = %lld",  i_pts );
        /* Send decoded frame to vout */
        if( i_pts > VLC_TS_INVALID)
        {
        //__android_log_print(ANDROID_LOG_DEBUG, "hoperun","i_pts > VLC_TS_INVALID");
            p_pic->date = i_pts;
	...
	__android_log_print(ANDROID_LOG_DEBUG, "hoperun","return p_pic");
            return p_pic;
        }
        else
        {
        __android_log_print(ANDROID_LOG_DEBUG, "hoperun","decoder_DeletePicture");
